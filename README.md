...
- __TODO:__ The Watch task currently only updates file additions not deletions, so this would be a good feature to implement. (specially for the images folders)
- __TODO:__ When the gulp master task is run, the overwrite.scss file is copied from the src and temporarily pasted into the base_template folder, this then gets copied over to the right folder in the src folder generated after sizes.json/Master is turned to false. But the file should then be deleted from the base_template folder. 
